...
import com.me.umtograph.graph.GraphWorker;
...
public class XMLVisioWorker implements XMLWorker {
	...
	private void addSpapeToList(Node node) {
		try {
			...
			if (shapeType != null) {
				shape.setType(shapeType.getType());
				shapeList.add(shape);
			}
	...
	@Override
	public void setMasters(File xml) {
		try {
		DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
		DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
		Document doc = dBuilder.parse(xml);
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			Element element = (Element) node;
			String uName = element.getAttribute(NAMEU).toString();
			int master = Integer.parseInt(element.getAttribute(ID));
			ShapeType.getByMasterOrName(master, uName);
		}
		}catch(Exception e) {
			e.printStackTrace();
		}
